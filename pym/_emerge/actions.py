...
def action_sync(settings, trees, mtimedb, myopts, myaction):
	...
	if portage.process.find_binary("git") is None:
		...
		for l in msg:
			writemsg_level("!!! %s\n" %1, level=logging.ERROR, noiselevel=-1)
		return 1
	updatecache_flg = True
	if not os.path.exists(myportdir):
		...
		print("\n>>> Using %s as temporary clone directory..." % repo_path_tmp)
		...
		for checkdir in [ repo_path_tmp, work_path ]:
			if os.path.exists(checkdir):
				try:
					...
					print("!!! %s exists and contains file. Please remove so that clone can proceed." % checkdir)
					sys.exit(1)
					
		if not os.path.exists(common_path):
			# Create common path directory, ie. /var/git, if it does not exist.
		...
		if portage.process.spawn_bash("su - %s -s /bin/sh -c 'cd %s'" % (syncuser, common_path)) != os.EX_OK:
			print("!!! Path %is is not reachable by user %s; please adjust permissions or SYNC_USER setting to correct." % (common_path, syncuser))
			sys.exit(1)
		print(">>> Staring initial git clone with " + syncuri + "...")
		
		if portage.process.spawn_bash("su - %s -s /bin/sh -c 'umask %s && cd %s && exec git clone --depth 1 --branch regen2 %s %s && mv '" % (syncuser, syncumask, work_path, portage._shell_quote(syncuri), repo_dir)) != os.EX_OK:
			print("!!! git clone error; exiting.")
			sys.exit(1)
		
		if portage.process.spawn_bash("su - %s -s /bin/sh -c 'umask %s && cd %s'" % (syncuser, syncumask, repo_path_tmp)) != os.EX_OK:
			print("!!! git checkout error; exiting.")
			sys.exit(1)
		...
		if not os.path.exists(myportdir + "/.git"):
			print("!!! Portage tree %s does not appear to be a git repository. Please move out of the way of correct your PORTDIR setting and retry." % myportdir)
			sys.exit(1)
		if portage.process.spawn_bash("su - %s -s /bin/sh -c 'cd %s > /dev/null 2>&1'" % (syncuser, myportdir)) != os.EX_OK:
			print("!!! Portage tree at %s is not reachable by user %s; please adjust permissions to corrent." % (myportdir, syncuser))
			sys.exit(1)
